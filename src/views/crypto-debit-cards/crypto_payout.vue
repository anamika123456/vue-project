                ...
                <div class="selectField border-left d-flex align-items-center selectCurrency">
                  <div class="selectDropMenu destinationDropMenu position-relative round-sm w-100">
                    ...
                    <ul class="dropdown-menu scrollbar">
                      ...
                      <li v-for="value in marginList()" v-show="marginList().length > 0" @click="currencyListNameFunc(value.currency, '')">
                        <a href="javascript:void(0)" class="">
                          <div class="d-flex align-items-center">
                            <img :src="'/assets/website/images/crypto_logos/'+value.currency.toLowerCase()+'.png'" alt="Coin" class="d-block cryptoCoin destinationdropImg h-auto" width="38" height="38" />
                            <div class="pl-1 pl-sm-2">
                              <!-- <p class="medium themecolor mb-0 text-left lh-normal" v-if="$env_var.restricted_currencies.hasOwnProperty(value.currency)">{{ $env_var.restricted_currencies[value.currency].currency_name}}</p> -->
                              <p class="medium themecolor mb-0 text-left lh-normal" >
                                {{ value.currency }}  - {{currencyNameFunc(value.currency)}}
                              </p>
                            </div>
                          </div>
                        </a>
...
<script>
  ...
  export default {
    ...
    mixins: [depositWithdrawalMixins, tradingMixins,cardsMixins],
    data() {
      return {
        pageLoader: false,
    ...
    methods: {
      ...
      marginList() {
        ...
        if(Object.keys(this.card_wallets).length > 0 && this.card_wallets.wallets && this.card_wallets.wallets.length > 0 &&  this.card_wallets.wallets[0].accounts && Object.keys(this.card_wallets.wallets[0].accounts).length > 0){
