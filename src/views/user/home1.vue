    ...
    <div class="mid_scroll_content">
      <div class="pageContent border-0">
        ...
        <div class="row mt-4">
          ...
          <div class="col-12 col-sm-12 col-lg-4 col-xl-4 col-xxl-4 d-flex" >
            <div class="border round-sm bg-white chartDiv-wrap w-100">
              <h4 class="heading">Wallets  </h4>
              <currencyChart  v-if="Object.keys(marginInfoList).length > 0 " />
              <div class="chartDiv-row chartDiv45" v-else>
                <no-data class="position-absolute w-100" ></no-data>
              </div>
            </div>
          </div>
        </div>
      ...
      totalValue(getCurrency) {
        try {
          ...
          for (const item in this.marginInfoList) {
            amount += parseFloat(this.getConvertedValue(this.marginInfoList[item], getCurrency));
          }
          return amount;
        } catch (e) {
          return "N/A";
