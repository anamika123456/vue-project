...
export class WsConnector extends Process implements IProcess {
  ...
  private buildQuery() {
    ...
    if (!this.settings.hasOwnProperty('query')) {
      return q
    }
    if ('query' in this.settings) {
      for (let key in this.settings.query) {
  ...
  public send(data: any, localParams: any) {
    if (this.ws.readyState === 1) {alert('if send'+this.ws.readyState)
      if (localParams !== undefined) { localParams.successWs() }
      console.log('return==',this.ws.send(data))
      ...
      this.waitForSocketConnection(this.ws, data, (data) => {
        try {
          if (this.ws.readyState === 1) {alert('send'+this.ws.readyState)
            if (localParams !== undefined) { localParams.successWs() }
            this.log(WsConnector.TAG, 'send',{ endpoint: this.settings.ENDPOINT, data })
            
            return this.ws.send(data)
            console.log('return 1===',WsConnector.TAG, this.settings.ENDPOINT,  data)
            console.log('return==',this.ws.send(data))
          } else {alert('send else')
