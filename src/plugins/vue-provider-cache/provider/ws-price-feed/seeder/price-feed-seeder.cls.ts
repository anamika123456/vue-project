...
export class PriceFeedSeeder extends Process implements ISeeder {
   ...
   */
  public watchlist(ids:any):void {alert(ids)
  ...
  public onmessage = (ev:MessageEvent) => { 
    try {
      ...
      const vuexStore = _that.iProvider.getStore()
      // vuexStore.commit(Const.SET_WS_STATUS, 1)  // 0:'yellow',1:'green',2:'orange',3:'red'
      const responseBuffer = Buffer.from(ev.data, 'base64');
          ...
          if (Object.keys(jsonMsg).length != 0) {
            ...
            if ("message" in jsonMsg) {
              ...
              if ("tempOrderId" in jsonMsg.message) {
                // console.log("message", jsonMsg.message);
              ...
              if ("orderupdate" in jsonMsg.message) {
                ...
                if (orderUpdateData.Status == "CREATED") {
                  ...
                  if ("Commission" in orderUpdateData) {
                    ...
                    temp["Commission"] =
                      (orderUpdateData.QuantityFilled * 0.5) / 100;
                  }
                }
                ...
                if (orderUpdateData.Status == "FILLED") {
                  ...
                  if (tempOldValue != "") {
                    if ("Commission" in orderUpdateData) {
                      ...
                      tempOldValue["Commission"] =
                        (orderUpdateData.QuantityFilled * 0.5) / 100;
                    }
                    tempOldValue["QuantityFilled"] = parseFloat(
                      orderUpdateData.QuantityFilled
                    ).toFixed(8);
                    tempOldValue["QuantityRemaining"] = parseFloat(
                      orderUpdateData.QuantityRemaining
                    ).toFixed(8);
                    tempOldValue["PriceFilled"] = parseFloat(
                      orderUpdateData.PriceFilled
                    ).toFixed(8);
                ...
                if (
                  orderUpdateData.Status == "CREATED" &&
                  temp["OrderType"] == "SL"
                ) {
                ...
                var openlist = orderList.records.filter(
                  (val) => val.Status == "CREATED"
                );
                var canclelist = orderList.records.filter(
                  (val) => val.Status == "CANCELED"
                );
                var filledlist = orderList.records.filter(
                  (val) => val.Status == "FILLED"
                );
                let stateData = {
