...
(function (definition) {
	...
	quoteFeedSimulator.url="https://pricefeed.capitalwallet.com/api/provider/charting";
	...
	quoteFeedSimulator.fetchInitialData=function (symbol, suggestedStartDate, suggestedEndDate, params, cb) {
		...
		var queryUrl = this.url +
			...
			"&interval=" + getInterval(params.period,params.interval);// params.period + params.interval;
			// +"&period=" + (params.period);
		...
		CIQ.postAjax(queryUrl, null, function(status, response){
			...
			if(status==200 && Array.isArray(JSON.parse(response).data)){ // if successful response from datafeed
	...
	quoteFeedSimulator.fetchUpdateData=function (symbol, startDate, params, cb) {
		...
		var queryUrl = this.url +
			...
			"&enddate=" + ''+
			"&interval=" + getInterval(params.period , params.interval);
		...
		CIQ.postAjax(queryUrl, null, function(status, response){
			// process the HTTP response from the datafeed
			if(status==200 && Array.isArray(JSON.parse(response).data)){ // if successful response from datafeed
				var newQuotes = quoteFeedSimulator.formatChartData(response,params.chart.dataSet);
	...
	quoteFeedSimulator.fetchPaginationData=function (symbol, suggestedStartDate, endDate, params, cb) {
		...
		CIQ.postAjax(queryUrl, null, function(status, response){
			...
			if(status==200 && Array.isArray(JSON.parse(response).data)){ // if successful response from datafeed
				...
				cb({quotes:newQuotes, moreAvailable:suggestedStartDate.getTime()>0, attribution:{}}); // return fetched data (and set moreAvailable)
			} else { // else error response from datafeed
				cb({error:(response?response:status)});	// specify error in callback
			}
		});
		...
		if(typeof(feeddata.data) != "undefined" && feeddata.data){
			...
			for(var i=0;i<feedDataArray.length;i++){
				try{
					...
					// utility function to format data for chart input; given simulator was designed to work with library, very little formatting is needed
					newQuotes[i].Date=quoteFeedSimulator.formatDate(feedDataArray[i].time); // DT is a string in ISO format, make it a Date instance
	...
	quoteFeedSimulator.formatDate=function (getInterval) {
		...
		UTC_DATE_FORMAT = getYear+"-"+getMonth+"-"+getDate+"T"+getHours+":"+getMins+":00.000Z"
